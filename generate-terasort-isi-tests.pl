...
foreach my $numComputeNodes (6) {                  # should normally decrease                
foreach my $blockSizeMiB (512) {                    # best is 512
foreach my $hdfsThreads (255) {                      # no effect
...
foreach my $smartCache (1) {                        # best is 1 (enabled)
    ...
    # Teragen
    foreach my $repeat_gen (1..0) {
    foreach my $mapCores_gen (0) {
    foreach my $mapMemoryMB_gen (32) {            # 
    foreach my $mapTasksPerNode (16, 8) {
    ...
    foreach my $reduceTasks (30, 300, 54, 100) {                     # best is 300 or 100, depending on compute node count
        # Terasort
        foreach my $reduceMemoryMB (2048) {                  # best is 2048
        foreach my $sortFactor (3000, 300) {                 # best is 30 but 30-3000 have nearly the same result
